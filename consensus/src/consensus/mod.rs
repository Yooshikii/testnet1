...
impl Consensus {
    ...
    fn pruning_samples_database_upgrade(&self) {
        ...
        let mut processed = 0;
        vecno_core::info!("Upgrading database to include and populate the pruning samples store");
        ...
        vecno_core::info!("Done upgrading database (populated {} entries)", processed);
    }
...
impl ConsensusApi for Consensus {
    ...
    fn calc_transaction_hash_merkle_root(&self, txs: &[Transaction], pov_daa_score: u64) -> Hash {
        let storage_mass_activated = self.config.starlight_activation.is_active(pov_daa_score);
