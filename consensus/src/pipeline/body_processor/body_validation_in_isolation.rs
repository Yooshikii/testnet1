...
impl BlockBodyProcessor {
    pub fn validate_body_in_isolation(self: &Arc<Self>, block: &Block) -> BlockProcessResult<Mass> {
        let starlight_activated = self.starlight_activation.is_active(block.header.daa_score);

        Self::check_has_transactions(block)?;
        Self::check_hash_merkle_root(block, starlight_activated)?;
        Self::check_only_one_coinbase(block)?;
        self.check_transactions_in_isolation(block)?;
        self.check_coinbase_has_zero_mass(block, starlight_activated)?;
        let mass = self.check_block_mass(block, starlight_activated)?;
    ...
    fn check_hash_merkle_root(block: &Block, starlight_activated: bool) -> BlockProcessResult<()> {
        let calculated = calc_hash_merkle_root(block.transactions.iter(), starlight_activated);
    ...
    fn check_coinbase_has_zero_mass(&self, block: &Block, starlight_activated: bool) -> BlockProcessResult<()> {
        // TODO (post HF): move to check_coinbase_in_isolation
        if starlight_activated && block.transactions[0].mass() > 0 {
            return Err(RuleError::CoinbaseNonZeroMassCommitment);
        }
        Ok(())
    }

    fn check_block_mass(self: &Arc<Self>, block: &Block, starlight_activated: bool) -> BlockProcessResult<Mass> {
        if starlight_activated {
