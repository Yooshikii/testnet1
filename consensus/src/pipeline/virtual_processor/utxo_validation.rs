...
pub(crate) mod starlight {
    ...
    #[derive(Clone)]
    pub(crate) struct StarlightLogger {
        steps: Arc<AtomicU8>,
    }
    ...
    impl StarlightLogger {
        ...
        pub fn report_activation(&self) -> bool {
            if self.steps.compare_exchange(Self::ACTIVATE, Self::ACTIVATE + 1, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
                info!(target: STARLIGHT_KEYWORD, "[Starlight] [--------- Starlight activated for UTXO state processing rules ---------]");
                true
            } else {
                false
            }
...
impl VirtualStateProcessor {
    ...
    pub(super) fn calculate_utxo_state<V: UtxoView + Sync>(
        ...
        if !self.starlight_activation.is_active(pov_daa_score) {
    ...
    fn verify_header_pruning_point(
        ...
        // [Starlight]: changing expected pruning point check from header validity to chain qualification.
        ...
        if self.starlight_activation.is_within_range_from_activation(header.daa_score, 1000) {
            self.starlight_logger.report_activation();
        }

        let selected_parent_daa_score = self.headers_store.get_daa_score(ghostdag_data.selected_parent).unwrap();
        // [Starlight]: we need to save reply.pruning_sample to the database also prior to activation
        let reply = self.pruning_point_manager.expected_header_pruning_point_v2(ghostdag_data);
        if self.starlight_activation.is_active(selected_parent_daa_score) {
    ...
    fn verify_coinbase_transaction(
        ...
        let expected_coinbase = self
            ...
            .unwrap()
            .tx;
        // [Starlight]: we can pass include_mass_field = false here since post activation coinbase mass field
    ...
    pub(super) fn validate_transaction_in_utxo_context<'a>(
        ...
        match res {
            ...
            Err(tx_rule_error) => {
                ...
                // [Starlight]: the above suggested aggregate seems not crucial for starlight since unupdated miners
    ...
    pub(super) fn calc_accepted_id_merkle_root(
        ...
        if self.starlight_activation.is_active(daa_score) {
