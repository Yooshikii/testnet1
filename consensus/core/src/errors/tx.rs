...
pub enum TxRuleError {
    ...
    #[error("transaction total inputs spending amount is higher than the max allowed of {}", MAX_VENI)]
    InputAmountTooHigh,

    #[error("transaction output {0} has zero value")]
    TxOutZero(usize),

    #[error("transaction output {0} value is higher than the max allowed of {}", MAX_VENI)]
    TxOutTooHigh(usize),

    #[error("transaction total outputs value overflowed u64")]
    OutputsValueOverflow,

    #[error("transaction total outputs value is higher than the max allowed of {}", MAX_VENI)]
    TotalTxOutTooHigh,
