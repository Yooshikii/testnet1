...
impl IbdFlow {
    ...
    async fn determine_ibd_type(
        ...
        // [Starlight]: use the post starlight pruning depth depending on hst's DAA score.
        ...
        if relay_header.blue_score >= hst_header.blue_score + pruning_depth && relay_header.blue_work > hst_header.blue_work {
            // [Starlight]: switch to the new *shorter* finality duration only after sufficient time has passed
