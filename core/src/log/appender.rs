...
impl AppenderSpec {
    pub fn console(name: &'static str, level: Option<LevelFilter>) -> Self {
        Self::new(
            name,
            level,
            Box::new(ConsoleAppender::builder().encoder(Box::new(StarlightEncoder::new(LOG_LINE_PATTERN_COLORED))).build()),
        )
    }
...
pub const STARLIGHT_KEYWORD: &str = "starlight";
const STARLIGHT_LOG_LINE_PATTERN_COLORED: &str = "{d(%Y-%m-%d %H:%M:%S%.3f%:z)} [{h({(CRND):5.5})}] {m}{n}";

// TODO (post HF): remove or hide the custom encoder
#[derive(Debug)]
struct StarlightEncoder {
    general_encoder: PatternEncoder,
    starlight_encoder: PatternEncoder,
    keyword: &'static str,
}

impl StarlightEncoder {
    fn new(pattern: &str) -> Self {
        StarlightEncoder {
            general_encoder: PatternEncoder::new(pattern),
            starlight_encoder: PatternEncoder::new(STARLIGHT_LOG_LINE_PATTERN_COLORED),
            keyword: STARLIGHT_KEYWORD,
        }
    }
}

impl Encode for StarlightEncoder {
    fn encode(&self, w: &mut dyn Write, record: &log::Record) -> anyhow::Result<()> {
        if record.target() == self.keyword {
            ...
            self.starlight_encoder.encode(w, &record)?;
